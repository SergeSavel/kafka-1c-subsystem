...
Функция XmlСериализовать(Значение, ОтступИлиЗаписьXML=Ложь) Экспорт
...
КонецФункции
Функция XmlПрочитатьСоответствие(ЧтениеXML) // доработать!
...
Процедура AvroЗаписатьFixed(ЗаписьXML, Значение, Имя=Неопределено, ПространствоИмен=Неопределено) Экспорт
	...
	Если ЗначениеТип = Тип("ДвоичныеДанные") Тогда
		ЗначениеСтрока = ПолучитьHexСтрокуИзДвоичныхДанных(Значение);
	ИначеЕсли ЗначениеТип = Тип("УникальныйИдентификатор") Тогда
		м = СтрРазделить(Строка(Значение), "-");
		ЗначениеСтрока = м[3]+м[4]+м[2]+м[1]+м[0];
	ИначеЕсли ЗначениеТип = Тип("Булево") Тогда
		ЗначениеСтрока = ?(Значение, "01", "00");
	ИначеЕсли ЗначениеТип = Тип("Строка") Тогда 
		ЗначениеСтрока = Значение; // передана hex-строка             
	Иначе
		ВызватьИсключение "Передано значение некорректного типа: """+ЗначениеТип+""".";
	КонецЕсли;
...
Процедура AvroЗаписатьDate(ЗаписьXML, Значение, Имя=Неопределено) Экспорт
	...
	Если ЗначениеТип = Тип("Дата") Тогда
		ЗначениеДата = НачалоДня(Значение);
	Иначе
		ВызватьИсключение "Передано значение некорректного типа: """+ЗначениеТип+""".";
	КонецЕсли;
...
Процедура AvroЗаписатьTimestampMillis(ЗаписьXML, Значение, Имя=Неопределено) Экспорт
	...
	ИначеЕсли ЗначениеТип = Тип("Число") Тогда // универсальныя дата в миллисекундах
		Миллисекунды = Цел(Значение % 1000);
		ЗначениеДата = '19700101' + Цел(Значение/1000);
		ЗначениеСтрока = XMLСтрока(ЗначениеДата);
		Если Миллисекунды <> 0 Тогда
			ЗначениеСтрока = ЗначениеСтрока+"."+Формат(Миллисекунды, "ЧРД=.; ЧГ=0; ЧФ=.Ч");
		КонецЕсли;
	Иначе
		ВызватьИсключение "Передано значение некорректного типа: """+ЗначениеТип+""".";
	КонецЕсли;
...
Процедура AvroЗаписатьTimestampMicros(ЗаписьXML, Значение, Имя=Неопределено) Экспорт
	...
	ИначеЕсли ЗначениеТип = Тип("Число") Тогда // универсальныя дата в микросекундах
		Микросекунды = Цел(Значение % 1000000);
		ЗначениеДата = '19700101' + Цел(Значение/1000000);
		ЗначениеСтрока = XMLСтрока(ЗначениеДата);
		Если Микросекунды <> 0 Тогда
			ЗначениеСтрока = ЗначениеСтрока+"."+Формат(Микросекунды, "ЧРД=.; ЧГ=0; ЧФ=.Ч");
		КонецЕсли;
	Иначе
		ВызватьИсключение "Передано значение некорректного типа: """+ЗначениеТип+""".";
	КонецЕсли;
...
Функция AvroПрочитатьFixed(ЧтениеXML)
	...
	Значение = ПолучитьДвоичныеДанныеИзHexСтроки(ЗначениеСтрока);
