...
Функция Produce(Topic, Partition=Неопределено, Value, Key=Неопределено, Headers=Неопределено) Экспорт
		
	HttpОтвет = Produce_(Topic, Partition, Value, Key, Headers);
	
	Возврат ПрочитатьТелоОтвета(HttpОтвет);
	
КонецФункции
Функция Produce_(Topic, Partition=Неопределено, Value, Key, Headers)
		
	Body = Новый Структура;
	Если Key <> Неопределено Тогда
		Body.Вставить("Key", Key);
	КонецЕсли;
	Если Headers <> Неопределено Тогда
		Body.Вставить("Headers", Headers);
	КонецЕсли;
	Body.Вставить("Value", Value);
...
// Удалить
Функция ПолучитьСрезПоследнихСообщенийТест(Тема)
	...
	Consumer = CreateConsumer(120000, ConsumerConfig);
	Если Consumer = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Попытка
		...
		Результат = ПолучитьСрезПоследнихСообщений_Тест(TopicMetadata, Consumer);
...
КонецФункции
//Удалить
Функция ПолучитьСрезПоследнихСообщений_Тест(TopicMetadata, Consumer)
	
	Назначение = Новый Массив;
	Для Каждого PartitionMetadata Из TopicMetadata.Partitions Цикл
		Назначение.Добавить(Новый Структура("Topic, Partition, Offset", TopicMetadata.Topic, PartitionMetadata.Partition, СпецСмещениеНачало()));
	КонецЦикла;
			
	Назначение = AssignConsumer(Consumer.Id, Назначение);
	Если Назначение = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ОставшиесяРазделы = Новый Соответствие;
	Для Каждого TopicPartiton Из Назначение Цикл
		ОставшиесяРазделы.Вставить(TopicPartiton.Partition, Истина);
	КонецЦикла;
	...
	Пока Истина Цикл
		...
		//_Момент1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
		...
		//_Момент2 = ТекущаяУниверсальнаяДатаВМиллисекундах();
		//Сообщить((_Момент2-_Момент1)/1000);
		
		Если ConsumerMessage = Null Тогда
			Прервать;
		КонецЕсли;
		
		Если ConsumerMessage.IsPartitionEOF Тогда
			ОставшиесяРазделы.Удалить(ConsumerMessage.Partition);
			Если ОставшиесяРазделы.Количество() = 0 Тогда
				Прервать;
			КонецЕсли;
			Продолжить;
		КонецЕсли;
...
Процедура ЗаписатьJSONвHttpЗапрос(HttpЗапрос, Значение)
	...
	ЗаписьJSON.ОткрытьПоток(HttpЗапрос.ПолучитьТелоКакПоток(), , , ПараметрыЗаписиJSON);
